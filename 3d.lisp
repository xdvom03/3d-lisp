...
(defun init (&optional (compile-p t))
  ...
  (if compile-p
      (progn
        ...
        (load "./ltk/ltk.fasl")
        (setf *wish-pathname* "./ltk/tcl/bin/wish")
        (in-package :ltk)
        ...
        (defparameter *fog* 500) ;Start of fog (division by zero protection)
        ...
        (compile-file "../prog/rotation.lisp") ;Mathy stuff about rotating in 3D space
        (load "../prog/rotation.lisp")
        (compile-file "../prog/3D-util.lisp") ;Fog etc.
        (load "../prog/3D-util.lisp"))))
...
(defun tick-compute (num widgets)
  ...
  (let ((c (make-instance 'canvas
    ...
    (bind *tk*
          "<Motion>"
          #'(lambda (event)
              (if (not (null *mouse-fix*))
                  (progn
                    (rot-cam-absolute (* -0.0003 (- (event-x event) (first *mouse-fix*))) (list 0 1 0))
                    (rot-cam-x (* 0.0003 (- (event-y event) (second *mouse-fix*))))))
              (apply #'set-cursor-pos (list 800 400))))
    (bind c
          "<Button-1>"
          #'(lambda (event)
              (setf *mouse-fix* (list (event-x event)
                                      (event-y event)))))
    (bind c
          "<x>"
          #'(lambda (a) (identity a) (set-cursor-pos 0 0)))
    (focus c)
    (create-objects num)
    (render c)
    (pack c)
